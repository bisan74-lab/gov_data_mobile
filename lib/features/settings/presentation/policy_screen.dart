    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: const [
          ...
          // **위치 좌표가 날씨 API로 전송된다는 사실을 빠뜨리면 안 된다.**
          // 앱 밖의 개인정보처리방침(스토어 제출용)·Play 데이터 보안 양식과
          // 내용이 어긋나면 심사에서 문제가 되고, 무엇보다 사실과 다르다.
          _Section(
            title: '5. 개인정보',
            body:
                '앱은 회원가입을 요구하지 않으며, 지역 선택·즐겨찾기·설정 값은 이용자 기기 내에만 '
                '저장됩니다. 개발자는 이러한 정보를 서버로 수집하지 않습니다.\n\n'
                '위치 권한은 선택 사항이며 바람나침판과 "현재 위치로 설정"에서만 사용합니다. '
                '날씨 예보를 받아오기 위해 위도·경도 좌표가 Open-Meteo와 공공데이터포털(기상청)로 '
                '전송되며, 이때 이용자를 특정할 수 있는 정보는 함께 보내지 않습니다. '
                '개발자는 전송된 좌표를 저장하지 않습니다.\n\n'
                '광고가 포함된 버전에서는 광고 제공자(Google AdMob)가 별도의 정책에 따라 '
                '광고 식별자 등을 수집할 수 있습니다.',
          ),
          _Section(
            title: '6. 지식재산권',
            body:
