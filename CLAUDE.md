...
2. **골프 커스터마이징이 섞인 파일**(`weather_screen.dart`): `// GOLF:` 주석이
   ...
   **상세 예보 표에서 너울·너울2·파력·수온 네 줄은 뺐다**(`rows`와 그에
   ...
   지도 시각 관련 규칙 셋(바다윈디에서 제보로 고친 것을 그대로 가져왔다):
   - **표를 닫을 때 지도 시각을 "지금"으로 되돌리지 않는다**(`_closeDetail`).
     표가 열려 있는 동안 `_syncMapHour`가 이미 지도 시각을 표의 선택과 맞춰
     두므로 되돌림 자체가 불필요했고, 보던 시각이 닫는 순간 사라져 보였다.
   - **표는 지도에서 고른 시각부터 연다**(`_PointForecastPanel.initialTime`).
     "지금"부터 열면 그 순간 `_syncMapHour`가 지도까지 "지금"으로 끌어내려,
     애써 옮겨 둔 날짜가 사라진다.
   - **히트맵 재굽기는 [LatestOnlyRunner]로 한 번에 하나만 돌린다**
     (`core/utils/latest_only_runner.dart`). 시간 슬라이더는 `divisions`가
     없어 드래그 한 번에 시각이 수십 번 바뀌는데, 바뀔 때마다 아이솔레이트를
     새로 스폰하면 그 스폰·직렬화 경합만으로 프레임이 밀린다. 그래서
     `_doRebuildHeatmap`은 **실제로 도는 시점**의 `widget.field`를 다시
     읽어야 한다(코디네이터가 값을 들고 다니지 않는다).
   위 셋의 회귀 테스트는 `weather_screen_test.dart`에 있다. **지도 시각을
   옮기는 폭을 줄이지 말 것** — 3시간만 옮기면 표의 3시간 칸에서 "지금
   기준"과 "지도 기준"이 같은 칸으로 떨어지는 시간대가 생겨, 고쳐도 안
   고쳐도 통과하는 검사가 된다(실제로 그렇게 새 버렸다).
...
- **바람나침판의 방위 계산은 직접 한다**(`core/sensors/heading_math.dart`):
  ...
  **순수 함수라 플러그인 없이 테스트된다** — 값을 바꾸면
  ...
  - **자기장 세기로 왜곡을 감지해 안내한다**(`isFieldPlausible`, 지구
    자기장 25~65µT). 자동차·철제 책상·자석 케이스 옆에서는 방위가 크게
    틀어지는데, **방향만 봐서는 못 잡는다** — 왜곡된 자기장도 방향은 멀쩡해
    보이고 방위각까지 똑같이 나온다. 판정은 **거르기 전 원값**으로 해야
    순간적인 접근이 평균에 안 묻힌다.
  - **가속도계는 아주 느리게 거른다**(`_accelAlpha` 0.08). 여기서 뽑는 건
    중력(아래가 어디냐)인데, 손으로 돌리는 동안의 **가로 가속도**가 섞이면
    중력 방향이 기울고 그만큼 계산된 북이 흔들린다("폰을 돌릴 때마다 동쪽이
    조금씩 달라진다" 제보, 2026-08-09). 중력은 원래 거의 안 변하니 느리게
    걸러도 잃을 게 없다.
  원판·화살표는 **사용자가 준 그림**(`tool/source_art/`)에서 만든 이미지다
  (`assets/compass/`, 재생성은 `python3 tool/make_compass_assets.py`).
  원본을 그대로 못 쓰는 이유가 둘 있다: **흰 배경이 불투명**해서 그대로
  겹치면 화살표의 흰 사각형이 나침반을 가리고, 나침반이 흑백 선화라 어두운
  테마에서 검은 선이 배경에 묻힌다. 그래서 나침반은 **알파 마스크**로
  바꿔(검은 선일수록 불투명, 색은 흰색) `Image.asset(color:)`으로 테마 색을
  입히고 — 한 장이 밝은/어두운 테마를 다 처리한다 — 화살표는 흰 바탕에 섞인
  것을 역산해 파란색을 복원했다(단순히 "흰색 지우기"로 하면 경계에 흰
  테두리가 남는다). 원본이 505×499라 **정사각형으로 패딩**했다: 원판 중심이
  위젯 중심과 어긋나면 회전할 때 나침반이 비틀거린다. 화살표 원본은 왼쪽을
  가리켜서 90도 돌려 **아래쪽**을 향하게 저장했다.
  원판은 **통째로** 돌리고(`screenRotationRad(0, heading)`) 화살표도 같은
  함수로 돌린다 — 두 각도가 어긋나면 화살표만 맞고 N 글자가 딴 데를
  가리킨다. 그림째 도니 남쪽을 볼 때 S 글자가 뒤집히는데, 실제 자침 나침반
  카드가 도는 방식과 같고, 글자로 읽어야 할 때를 위해 화면 아래에 한글 풍향
  표시를 따로 둔다.
  **위치 조회에는 반드시 시간 제한을 건다** — 없으면 실내 첫 측위처럼 위치
  서비스가 응답하지 않을 때 "확인하는 중…"에서 영영 안 벗어난다(테스트가
  이 상태를 재현한다).
  화면 하단 **"방향이 정확해야 할 때"** 안내(8자 보정·금속 회피 등)는
  접이식이다 — 늘 펼쳐 두면 원판이 그만큼 작아진다.
